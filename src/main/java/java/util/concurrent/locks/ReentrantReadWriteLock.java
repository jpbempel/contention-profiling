...
public class ReentrantReadWriteLock
    ...
    static Map<ReentrantReadWriteLock, String> locks = Collections.synchronizedMap(new LinkedHashMap<ReentrantReadWriteLock, String>());
    static AtomicInteger lockIdGen = new AtomicInteger(0);
    static ConcurrentLinkedQueue<LockInfo> queue = new ConcurrentLinkedQueue<ReentrantReadWriteLock.LockInfo>();
    ...
    static class LockInfo
    {
        ...
        LockInfo(String id, ReentrantReadWriteLock lock, StackTraceElement[] stack)
        {
            this.id = id;
            this.lock = lock;
            this.stack = stack;
    ...
    static
    {
        Thread t = new Thread("ReentrantReadWriteLock stats")
        {
            @Override
            public void run()
            {
                File statsFile = new File("./ReentrantReadWriteLockStats.txt");
                        ...
                        while (!isInterrupted())
                        {
                            try
                            {
                                ...
                                while ((lockInfo = queue.poll()) != null)
                                {
                                    ...
                                    for (int i = 3; i < lockInfo.stack.length; i++)
                                        sb.append(lockInfo.stack[i]).append(", ");
                                }
                                synchronized (locks)
                                {
                                    for (Map.Entry<ReentrantReadWriteLock, String> entry : locks.entrySet())
                                    {
                                        ReentrantReadWriteLock reentrantLock = entry.getKey();
    ...
    int getContentedCount()
    {
        return sync.contented;
    }
